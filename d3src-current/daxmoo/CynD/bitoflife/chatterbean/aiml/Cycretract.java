...
public class Cycretract extends TemplateNodeTag {
    public static CycAccess cycAccess = null;
    /*
    Constructors
    */

    public Cycretract(Attributes attributes) {
    }

    public Cycretract(Object... children) {
        super(children);
    }
    ...
    */
    ...
    public String toString(Match match) {
        ...
        try {
            String cycResult;
            query="(cyc-query '"+aimlresult+")";
            java.lang.System.out.println("Cycretract query=["+query+"]");

            if (aimlresult.contains("?") ) {
            ...
            if (guessList instanceof CycList) {
                ...
                for (int i=0; i<((CycList)guessList).size();i++) {
                    ...
                    CycList bindingList1=(CycList)((CycList) guessList).get(i);
                    String removeQuery = "(cyc-unassert '"+aimlresult+")";
                    
                    for(int j=0; j<bindingList1.size();j++){
                        CycList pair2=(CycList)bindingList1.get(j);
                        String variableText =""+ pair2.get(0);
                        ...
                        java.lang.System.out.println("ResultFort "+i+","+j+ ":"+resultFort.cyclify ());
    
                        //result = getBinding(pair).cyclify();
                        cycResult=resultFort.cyclify();
                        removeQuery.replaceAll(variableText,cycResult);
                    }
                    java.lang.System.out.println("Cycretract1 removeQuery=["+removeQuery+"]");

                    Object removedList =cycAccess.converseObject(removeQuery);
                    bestResult=((CycObject)removedList).cyclify();
                }
            ...
            {
                String removeQuery = "(cyc-unassert '"+aimlresult+")";
                java.lang.System.out.println("Cycretract2 removeQuery=["+removeQuery+"]");
