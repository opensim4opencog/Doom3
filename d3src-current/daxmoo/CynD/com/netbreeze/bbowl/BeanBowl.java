 ...
 */
public class BeanBowl implements VetoableChangeListener,
	 ...
	 */
	public synchronized Object createAndAddBean(Class cl)
	...
	public synchronized boolean addBean(Object obj) {
		if (containsBean(obj)) {
			...
			BeanWrapper wrapper = new BeanWrapper(generateUniqueName(obj), obj);
			...
			Iterator it = bowlListeners.iterator();
			while (it.hasNext()) {
				// @temp
	...
	public synchronized boolean removeBean(Object obj) {
		if (containsBean(obj)) {
			...
			// Update the name index
			nameIndex.remove(wrapper.getName());
	 ...
	 */
	public synchronized void setSelectedBean(Object object)
			throws PropertyVetoException {
	...
	public void save(File destination) throws IOException {
		cat.debug("Saving bowl to " + destination);
	...
	static public BeanBowl load(File source) throws IOException,
	...
	private String generateUniqueName(Object object) {
		if (object instanceof INamedObject) {
			try {
				return ((INamedObject) object).getName();
			} catch (IOException e) {
		...
		while (!done) {
			...
			Object otherBean = findBean(name);
			if (otherBean == null) {
				done = true;
			} else {
				++counter;
