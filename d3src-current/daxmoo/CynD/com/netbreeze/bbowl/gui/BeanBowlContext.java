...
//==== Constructors ===================
   ...
   * All operations will use either the given GUI or the bean bowl
   * that it represents
   */
   ...
   */
   public void showBeanDetails(Object bean) throws Exception {
    ...
    if (existing == null || ALLOW_MULTIPLE_WINDOWS) {
      ...
      //Get a wrapper for the bean, or create a temporary wrapper if necessary
      ...
      Dimension size = new Dimension(
        Math.min(preferred.width, deskSize.width),
        Math.min(preferred.height, deskSize.height)
      );
      frame.setSize(size);

      //If necessary, add this to the list of bean frames
      //to allow reuse of this window if the same bean is to be viewed again
...
//==== Other public methods =========================
  ...
  /**
   * For beans that happen to be Components, this method
   * can be used to cause the bean to be drawn as a component.
   */
...
//===== Event adapter classes ==================================
  ...
  /**
   * Window event adapter class, used to find out when
   * child windows close
   */
