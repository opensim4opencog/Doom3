 ...
 */
/*
 * Thanks to Slava Pestov (of jEdit fame) for import caching enhancements. Note:
 * This class has gotten too big. It should be broken down a bit.
 */
public class NameSpace implements java.io.Serializable, BshClassManager.Listener, NameSource {
	...
	public Hashtable getVariables() {// dmiles
		return variables;
	}
	protected Hashtable importedClasses;
	public Hashtable getImportedClasses() {// dmiles
		return importedClasses;
	}
	public Vector getImportedObjects() {// dmiles
	 ...
	 */
	protected/* dmiles */boolean isClass;
	Class classStatic;
	 ...
	 *            specifies whether strict java rules are applied.
	 * @param recurse
	 *            determines whether we will search for the variable in our
	...
	void setVariable(String name, Object value, boolean strictJava, boolean recurse) throws UtilEvalError {
		...
		// No previous variable definition found here (or above if recurse)
		{
			...
			varScope.variables.put(name, createVariable(name, value, null/* modifiers */));
	...
	public String[] getVariableNames() {
		...
		else return enumerationToStringArray(variables.keys());
	}
	/**
	...
	public String[] getMethodNames() {
		...
		else return enumerationToStringArray(methods.keys());
	}
	/**
	...
	public BshMethod[] getMethods() {
		if (methods == null) return new BshMethod[0];
		else return flattenMethodCollection(methods.elements());
	}
	...
	private BshMethod[] flattenMethodCollection(Enumeration e) {
		...
		while (e.hasMoreElements()) {
			...
			if (o instanceof BshMethod) v.addElement(o);
			else {
				Vector ov = (Vector) o;
	 ...
	 * 
	 * Caching a single instance here seems technically incorrect, considering
	 * the declaringInterpreter could be different under some circumstances.
	 * (Case: a child interpreter running a source() / eval() command ). However
	 * the effect is just that the main interpreter that executes your script
	 * should be the one involved in call-backs from Java.
	 * 
	 * I do not know if there are corner cases where a child interpreter would
	 * be the first to use a This reference in a namespace or if that would even
	 * cause any problems if it did... We could do some experiments to find
	 * out... and if necessary we could cache on a per interpreter basis if we
	 * had weak references... We might also look at skipping over child
	 * interpreters and going to the parent for the declaring interpreter, so
	 * we'd be sure to get the top interpreter.
	 */
	...
	public void prune() {
		...
		// XXX if we keep the createClassManager in getClassManager then we can
		// axe
		// this?
	 ...
	 * 
	 * @param recurse
	 *            If recurse is true then we recursively search through parent
	 *            namespaces for the variable.
	 *            <p>
	 *            Note: this method is primarily intended for use internally. If
	 *            you use this method outside of the bsh package you will have
	 *            to use Primitive.unwrap() to get primitive values.
	 * @see Primitive#unwrap( Object )
	 * 
	 * @return The variable value or Primitive.VOID if it is not defined.
	 */
	 ...
	 * through parent name spaces. <p/> If this namespace is static, return only
	 * static variables.
	 * 
	 ...
	 * 
	 * @param name
	 *            is here just to allow the error message to use it protected
	 *            void checkVariableModifiers( String name, Modifiers modifiers )
	 *            throws UtilEvalError { if ( modifiers!=null &&
	 *            modifiers.hasModifier("static") ) throw new UtilEvalError(
	...
	public void setMethod(String name, BshMethod method) throws UtilEvalError {
		...
		if (methods == null) methods = new Hashtable();
		Object m = methods.get(name);
		if (m == null) methods.put(name, method);
	...
	public void importPackage(String name) {
		...
		if (importedPackages.contains(name)) importedPackages.remove(name);
	...
	public void importCommands(String name) {
		if (importedCommands == null) importedCommands = new Vector();
		// dots to slashes
		name = name.replace('.', '/');
		// absolute
		...
		// If it exists, remove it and add it at the end (avoid memory leak)
		if (importedCommands.contains(name)) importedCommands.remove(name);
	 ...
	 * may be imported using the importCommands() method. <p/>
	 * 
	 ...
	 *             if loadScriptedCommand throws UtilEvalError i.e. on errors
	 *             loading a script that was found
	 */
	...
	protected Variable getImportedVar(String name) throws UtilEvalError {
		...
		if (importedObjects != null) for (int i = 0; i < importedObjects.size(); i++) {
			...
			if (field != null) return createVariable(name, field.getType(), new LHS(object, field));
		}
		// Try static imports
		if (importedStatic != null) for (int i = 0; i < importedStatic.size(); i++) {
			Class clas = (Class) importedStatic.elementAt(i);
			Field field = Reflect.resolveJavaField(clas, name, true/* onlyStatic */);
			if (field != null) return createVariable(name, field.getType(), new LHS(field));
		}
		return null;
	}
	/**
	...
	private Class getImportedClassImpl(String name) throws UtilEvalError {
		...
		if (importedClasses != null) fullname = (String) importedClasses.get(name);
	 ...
	 * instead.
	 */
	protected /*dmiles*/ Name getNameResolver(String ambigname) {
	...
	public void importObject(Object obj) {
		if (importedObjects == null) importedObjects = new Vector();
		// If it exists, remove it and add it at the end (avoid memory leak)
		if (importedObjects.contains(obj)) importedObjects.remove(obj);
	...
	public void importStatic(Class clas) {
		if (importedStatic == null) importedStatic = new Vector();
		// If it exists, remove it and add it at the end (avoid memory leak)
		if (importedStatic.contains(clas)) importedStatic.remove(clas);
	...
	boolean attemptSetPropertyValue(String propName, Object value, Interpreter interp) throws UtilEvalError {
		...
		if (m != null) {
			try {
				invokeMethod(accessorName, new Object[]
					{ value }, interp);
	 ...
	 * @throws UtilEvalError
	 */
	protected/*dmiles*/ Object getPropertyValue(String propName, Interpreter interp) throws UtilEvalError {
		String accessorName = Reflect.accessorName("get", propName);
		Class[] classArray = new Class[0];
		BshMethod m = getMethod(accessorName, classArray);
		try {
			if (m != null) return m.invoke((Object[]) null, interp);
