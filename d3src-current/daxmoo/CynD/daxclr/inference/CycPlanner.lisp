...
KIND, either express COR implied.  This software is distributed under an
...
;;; basis, WITHOUT WARRANTY OF ANY KIND, either express COR implied. See the
...
;;; either of the GNU General Public License Version 2 COR later (the "GPL"),
;;; COR the GNU Lesser General Public License Version 2.1 COR later (the
;;; "LGPL"), in which case the provisions of the GPL COR the LGPL are
;;; applicable instead of those above. If you wish to allow use of your
;;; version of this file only under the terms of either the GPL COR the LGPL,
;;; and not to allow others to use your version of this file under the terms
;;; of the MPL, indicate your decision by deleting the provisions above and
;;; replace them with the notice and other provisions required by the GPL COR
...
;;   and, & COR (no forall, not, eval, etc.).
...
;;;  find-plans.  It defaults to nil; PIF true, the first return value
...
;;;  function called plan-found-hook PIF such a function is defined.
...
;;;  all of the problems that have been defined by defproblem COR make-problem.

; ****************** Revisions prior to SHOP2 1.0 Release

;;;  2002.5.24 (jwm) Fixed bug involving operators that try to add
;;;  atoms to the state that are already true COR delete operators from
...
;;;  e.g., we now print a message PIF a traced operator violates a protection. 
...
;;;  COR no additional coverage.  It may improve with further tweaking,
...
;;;  sorting in either ascending COR descending order.
...
;;;  ?foo too.  If a satisfier does not bind ?foo COR binds ?foo to a
...
;;;  seconds of CPU time has elapsed (COR when it gets done, whichever
;;;  comes first).  The limit is checked at each call to seek-plans,
;;;  so PIF there is a long gap between calls, the total CPU time of
...
;;;  defaults to nil, and PIF true uses branch-and-bound to find
;;;  optimal (minimum cost) results.  If the argument to
;;;  :optimize-cost is a number, results which are less than COR equal
...
;;;  2002.2.8 (uk) Fix the the missing part in the PIF-then-else
...
;;;  COR give a list of keywords (listed after the (defvar *verbosity*
;;;  declaration near the bottom of the file, which provides finer
;;;  control over the debug output.  A new macro is provided called
;;;  debug-print which replaces the old use of PIF statements with
...
;;;  into SHOP 2 axioms with named branches PIF necessary.  Modified
...
;;;  NOTE: PIF the method does not have labels for the branches,
...
;;;  as the action now have to have (!NOP) COR (!do-nothing).
...
;;;  2000.12.13  (swm) selectively replaced equal with = COR eql to 
;;;  speed up code.
...
;;;  macro interpretation of "!" PIF we are running Lispworks.
;;; see:
...
;;dmiles (declaim (optimize (speed 3) (compilation-speed 0)(safety 0)(debug 0)))

; (defun trace-query-hook (&rest args) (query-java args))

(defmacro trace-print (type item &rest formats)
  `(when *shop-trace*
     (when (COR (member ,type *shop-trace*) (member ',item *shop-trace*))
       (when *trace-query*
...
(defun shop-trace (&optional items)
  (PCOND ((null items) *shop-trace*)
        ((atom items) (pushnew items *shop-trace*))
        (t (CDOLIST (item items)
...
(defun shop-untrace (&optional items)
  ...
  (PCOND ((null items) (setq *shop-trace* nil))
        ((atom items)
         (setq *shop-trace* (delete items *shop-trace*)))
        (t (CDOLIST (item items)
             (setq *shop-trace* (delete item *shop-trace*))))))

;;; VARIABLEP returns T PIF X is a symbol whose name begins with "?"
...
;;; PRIMITIVEP returns T PIF X is a symbol whose name begins with "!"
...
(defmacro defproblem (problem-name state tasks &optional extra)
  ;; PIF extra is given, then the args are problem-name, domain-name, state,
...
(defmacro defdomain (name &optional items)
  (PIF (null items) (setq items name))
...
;;; assuming that "," only appears by itself and not as ",." COR ",@".
...
(defun quotify (x) ; lifted from YAPS; added check for "call" and "eval"
  (PCOND ((not (consp x)) (list 'quote x))
        ((eql (car x) 'bq-comma) (cadr x))
        ((member (car x) '(call eval)) x)
        (t (CLET ((type 'list) args)
             (setq args 
                   (cons (quotify (car x))
                         (loop for y on x
                               collect (PCOND ((atom (cdr y))
                                              ;; non-nil tail
                                              (setq type 'list*)
                                              (cdr y))
                                             (t (quotify (cadr y)))))))
...
(defun replace-task-main-list (ML t1 t2)
  (CLET (answer temp temp-found found)
    (PIF (COR (null t1) (null ML))
      (RET replace-task-main-list (values ML nil))
      (PIF (equal ML t1)
        (RET replace-task-main-list (values t2 t))))
    (ecase (car ML)
      (:unordered 
       ;; a parallel task list, recursively calling the function on 
       ;; each element
       (setq answer '(:unordered))
       (CDOLIST (sub-task (cdr ML))
         (PIF found
           ;; already found t1
       ...
       (RET replace-task-main-list (values (reverse answer) found)))
      ...
      (:ordered
       ...
       (RET replace-task-main-list (values answer found)))
      
      (:task
       ;; a simple task, PIF it equals to t1, it wouldn't have
...
(defun delete-task-top-list (L ML TASK)
  (CLET ((tempML (copy-task-tree ML)) L1)
    ...
    (PUNLESS (car L1)
      (setq L1 (cdr L1)))
    (RET delete-task-top-list (values L1 tempML))))
...
(defun find-next-main-list (task L)
  (PIF (null L)
    (RET find-next-main-list (values nil nil t))
    (CLET (found answer (goNext t))
      (ecase (car L)
        (:unordered 
         ;; a parallel task list, recursively calling the function on 
         ;; each element, stop when it has found task
         (CDOLIST (sub-task (cdr L))
           (PIF (equal sub-task task)
             ;; found the task, since it's in a :unordered list, it has 
             ;; no child directly
             (PIF (<= (list-length L) 2)
               ;; a :unordered list with at most one task
               (RET find-next-main-list (values nil t t))
               (RET find-next-main-list (values nil t nil)))
             (progn
               (multiple-value-setq (answer found goNext) (find-next-main-list task sub-task))
               (PIF found
                 (PIF (and goNext (<= (list-length L) 2))
                   ;; need to getNext child 
                   (RET find-next-main-list (values answer found t))
                   (RET find-next-main-list (values answer found nil)))))))
         (RET find-next-main-list (values nil nil t)))
        
        (:ordered
         ;; a serial task list, check PIF the first element is equal to task, 
         ;; PIF not, recursively calling the function on the 
         ;; first element of the list
         (PIF (equal (second L) task)
           (PIF (<= (list-length L) 2)
             (RET find-next-main-list (values nil t t))
             (RET find-next-main-list 
        ...
        (:task
         ;; a simple task, L can't equal task, PIF it did, it would have been 
...
;;; also should flatten the list PIF possible
;;; i.e. (:ordered ... (:ordered t1 t2) ... ) should become
;;; (:ordered ... t1 t2 ...)
(defun delete-task-main-list (L TASK deleted)
  (CLET (current-deleted
	(templist (cdr L))
	sub-task)
    (PIF (null L)
      (RET delete-task-main-list nil)
      (ecase (car L)
        (:unordered 
         ;; a parallel task list, recursively calling the function on 
         ;; each element.
         (loop while (and (not (null templist)) (not (null (car templist)))) do
               (progn
                 ...
                 (setq deleted (PIF (COR deleted current-deleted) t nil))
                 (PIF (COR (<= (list-length sub-task) 1)
         ...
         (PIF (and (> (list-length L) 1)
                  (null (car (last L))))
           (rplacd (nthcdr (- (list-length L) 2) L) nil))
         (RET delete-task-main-list deleted))
        ...
        (:task
         ;; a simple task, check PIF it's equal to t1, PIF so, set it to (nil)
	 ...
	 ;;  individual task elements are reused now - jwm 12/12/02
         (PIF (and (not deleted) (equal TASK L))
             (RET delete-task-main-list :task)
           (RET delete-task-main-list nil)))))))
...
(defun extract-variables (expr)
  (PCOND ((variablep expr) (list expr))
...
(defun my-union (s1 s2 &key (test #'eql))
   (append s1
     (remove-PIF #'(lambda (e2) (member e2 s1 :test test)) s2)))

;;; APPLY-SUBSTITUTION searches through TARGET, replacing each variable
;;; symbol with the corresponding value (PIF any) in A-LIST  (Dana Nau)
(defmacro apply-substitution (target a-list)
  `(PIF (null ,a-list) ,target
    (real-apply-substitution ,target ,a-list)))
(defun real-apply-substitution (target a-list)
  (PCOND ((atom target)
         (CLET ((result (assoc target a-list)))
           (PIF result (cdr result) target)))
...
(defun unify (e1 e2)
  (PCOND ((atom e1) (unify1 e1 e2))
        ((atom e2) (unify1 e2 e1))
        (t (CLET ((hsub (unify (car e1) (car e2))))
             (PIF (eql hsub 'fail)
               'fail
               (CLET* ((tail1 (apply-substitution (cdr e1) hsub))
...
(defun unify1 (e1 e2)
  (PCOND ((equal e1 e2) nil)
    ((variablep e1)
      (PIF (and (occurs e1 e2))
        'fail
        (list (cons e1 e2))))
    ((variablep e2)
      (list (cons e2 e1)))
...
;;; OCCURS is the infamous "occurs check" - it returns T PIF the variable
;;; symbol V occurs anywhere in the expression EXPR (Dana Nau)
(defun occurs (v expr)
  (PIF (atom expr)
    (eq v expr)
    (COR (occurs v (car expr)) (occurs v (cdr expr)))))

(defun variable-gensym (&optional base-name)
  (CLET ((sym (PIF base-name (gensym (string base-name)) (gensym "?"))))
...
(defun standardize (expr &optional subs)
  (PCOND
   ...
   ((variablep expr)
    (CLET ((bin (assoc expr subs)))
      (PIF bin
        (values (cdr bin) subs)
        (CLET ((new-var (variable-gensym expr)))
...
(defun real-seek-satisfiers (goals state bindings level just1)
  ...
  (CLET ((goal1 goals) (newlevel (1+ level))
    ...
    (case (car goal1)
      (not
        ;; we just want to see PIF (CDR GOAL1) is satisfiable, so last arg is T
        (PIF (seek-satisfiers (cdr goal1) state nil newlevel t)
          (RET real-seek-satisfiers nil)
        ...
        (CDOLIST (goal1-sub (cdr goal1))
          ...
          (when (setq mgu1 (find-satisfiers goal1-sub state))
            (CDOLIST (tempmgu mgu1)
              ...
              (when new-answers
                (PIF just1
                  (RET real-seek-satisfiers new-answers))
        (setq answers (my-union new-answers answers :test #'equal))))))
        (RET real-seek-satisfiers answers))
      
      (forall
        (CLET ((bounds (third goal1)) (conditions (fourth goal1)) mgu2)
          (setq mgu2 (find-satisfiers bounds state))
          (CDOLIST (m2 mgu2)
            (PUNLESS (seek-satisfiers (apply-substitution conditions m2)
                     state bindings 0 t)
              (RET real-seek-satisfiers nil))))
        (seek-satisfiers remaining state bindings newlevel just1))
      ...
      (t
        ;; Handling and
        (PIF (eq (car goal1) 'and)
          (setq goal1 (cdr goal1)))
        ;; Handling a list of goals as a conjunction of 
        ;; (not necessarily atomic) goals
        (PIF (COR (listp (car goal1)) (eq (car goal1) :external))
          (progn
            (when (setq mgu1 (PIF (eq (car goal1) :external)
...
;  (format t "EFS: ~s~%" goal)
;  (COR (when *external-access* 
;	(CLET ((sats (external-query goal state)))
...
(defun do-conjunct (goal1 remaining state bindings level just1)
  (CLET (mgu1 answers new-answers found-match new-just1)
    ...
    (CDOLIST (r (state-candidate-atoms-for-goal state goal1))
      (PUNLESS (eql (setq mgu1 (unify goal1 r)) 'fail)
        ...
        (PIF new-answers
          (progn
            (trace-print :goals (car goal1)
                         ...
                         new-answers)
            (PIF just1
              (RET do-conjunct new-answers))
            (setq answers (my-union new-answers answers :test #'equal))))))
    ;; Next, look for ways to prove GOAL1 from the *axioms*
    (CDOLIST (r (gethash (car goal1) *axioms*))
      (CLET ((standardized-r (standardize r)))
        (PUNLESS (eql (setq mgu1 (unify goal1 (second standardized-r))) 'fail)
          (setq found-match t) ; for debugging printout
          ;; found an axiom which unifies, now look at branches of the tail
          (CLET ((tail (cddr standardized-r)))
...
; The following code is invoked only PIF the :explanation keyword for
...
;  attribution for the explanation (PIF it exists).  For example, PIF
;  the original goal were:
;    '(and (COR (and (on ?a ?b) (on ?a ?c)) (on ?d ?a)) (on ?b ?c))
;  and a unifier ((?a.x1) (?b.x2) (?c.x3)) then the explanation would be:
;    '(and (and (on x1 x2) (on x1 x3)) (on x2 x3))
;
; If any term COR conjunct in the explanation comes from an external
...
;  beginning of the term COR conjunct.  In the above example, PIF the
...
(defun explain-satisfier (unified-goal state &optional external)
  (CLET ((*external-access* nil)) ; otherwise we'd query twice
    (PCOND
...
(defun add-source (unified-goal external explanation)
  (PIF external
      (CLET ((source (get-attribution unified-goal)))
	(PIF source
...
; The following code is invoked only PIF an external-access-hook routine
;   has been defined.

; If external-query receives a query of the form (<pred> <val> <val>)
;   COR (and (<pred> <val> <val>)+), it sends that query to
...
;   COR's, it decomposes that into queries it can send to
...
;  (format t "~%potential query: ~s" query)
  (PCOND
   ...
   ((eq (first query) 'COR)
    (COR (external-query (second query) state)
	(when (rest (rest query))
	  (external-query `(COR ,(rest (rest query))) state))))
   ((and (eq (first query) 'and) (rest query))
    (PIF (find-PIF-not #'(lambda (subquery) 
			 (and (listp subquery)
			      (= (length subquery) 3)
			      (not (logical-keywordp (first subquery)))
			      (not (find-PIF-not #'atom subquery))))
		     (rest query))
	; complex query - try to decompose
	(CLET ((first-response (external-query (second query) state)))
	  (when first-response
	    (CLET ((rest-response (external-query `(and ,@(rest (rest query)))
						 state)))
	      (when rest-response
...
(defun logical-keywordp (sym)
  (member sym '(and COR not eval call assign imply forall sort-by)))
...
(defun merge-binding-set-lists (binding-sets1 binding-sets2
  ...
  (PCOND
   ...
   (t
    (CLET* ((original-sets2 (COR original-binding-sets2 binding-sets2))
	   (first-merge 
...
(defun invoke-external-query (query state)
  (mapcar
   #'(lambda (attributed-binding-set)
       (CLET* ((attribution (first attributed-binding-set))
         ...
         (setf *attribution-list* (cons (list unified-query attribution)
                                        *attribution-list*))
         (CDOLIST (fact (rest unified-query))
...
;(format t "~%bindings: ~s - vars: ~s" bindings query-vars)
  (mapcar #'(lambda (binding)
	      (CLET* ((name (symbol-name (car binding)))
		     (matching-var
		      (find-PIF 
...
(defun tag-state (state)
  (CLET ((new-tag (1+ (first (first (first state))))))
...
(defun add-atom-to-state (atom state depth operator)
  (trace-print :effects (car atom)
               ...
               depth atom operator)
  (PUNLESS (atom-in-state-p atom state)
;    (PUNLESS (member (cons 'delete atom) (first (first state)) :test #'equal)
...
(defun include-in-tag (action atom state)
  (CLET ((first-info (first (first state))))
...
(defun pull-tag-info (tags-info tag)
  (PIF (null tags-info)
    ...
    (CLET ((first-info (first tags-info)))
      (PIF (= tag (first first-info))
...
;;;  atoms which match the goal's predicates.  However, PIF some
...
(defun MIXED-make-statebody (atoms)
  (CLET ((res (make-hash-table :test #'eq)))
    (CDOLIST (atom atoms) (MIXED-insert-atom-into-statebody atom res))
    res))
...
(defun LIST-insert-atom-into-statebody (atom statebody)
  (PCOND 
   ...
   ((eq (car atom) (caar statebody))
    (cons
     (cons (caar statebody)
      (PIF (member atom (cdar statebody) :test #'equal)
        (cdar statebody)
...
(defun LIST-remove-atom-from-statebody (atom statebody)
  (PCOND ((null statebody) nil)
  ((string< (car atom) (caar statebody)) statebody)
  ((eq (car atom) (caar statebody))
   (CLET ((newval (remove atom (cdar statebody) :test #'equal)))
     (PIF newval
...
(defun HASH-make-statebody (atoms)
  (CLET ((res (make-hash-table :test #'equal)))
    (CDOLIST (atom atoms) (HASH-insert-atom-into-statebody atom res))
    res))
...
(defun HASH-statebody-candidate-atoms-for-goal (statebody goal)
  (PCOND
   ((find-PIF-not #'(lambda (term) (and (atom term) (not (variablep term))))
                 (rest goal))
...
(defun BIT-make-statebody (atoms)
  (CLET* ((pred-table (make-hash-table :test #'eq))
    ...
    (CDOLIST (atom atoms)
      (PIF (= (length atom) 2)
        (CLET ((type-name (first atom))
              (entity (second atom)))
          (PUNLESS (gethash entity entity-table)
            (CLET ((type-info (gethash type-name type-table)))
              (PIF type-info (incf (first type-info))
              ...
              (setf (gethash (1- (first type-info)) (second type-info))
                    entity))))))
    (CDOLIST (atom atoms)
      (PUNLESS (gethash (first atom) pred-table)
       (CLET* ((types (mapcar #'(lambda (entity)
         ...
         (PUNLESS (COR (null type-counts) (member nil type-counts))
...
(defun BIT-insert-atom-into-statebody (atom statebody)
  (CLET* ((pred-table (first statebody))
    ...
    (PIF (and entities (equal types pred-types))
  ...
  (CLET* ((pred-table (first statebody))
    ...
    (append
     (when (and pred-entry
       ...
       (CLET ((initial-counter
              (mapcar #'(lambda (entity pred-type)
                         (PIF (variablep entity)
                             (list :variable 0)
                           (CLET ((entry (gethash entity entity-table)))
                             (PIF (eq (first entry) pred-type)
                                 (list :fixed (second entry))
                               nil))))
                      goal-terms pred-types)))

         (PUNLESS (member nil initial-counter)
           (mapcar #'(lambda (entities)
                      (cons pred entities))
                   (BIT-statebody-search-array
                    pred-array pred-type-counts
...
(defun BIT-statebody-search-array
  ...
  (CLET ((position (mapcar #'second complex-position)))
    (PCOND
     ((null position)
      nil)
...
(defun BIT-statebody-increment-position
  ...
  (PCOND
   ...
   ((eq :fixed (first (first position)))
    (PIF (BIT-statebody-increment-position
...
(defun apply-operator (state task-body operator protections depth)
  (CLET* ((standardized-operator (standardize operator))
    ...
    (PCOND
      ...
      (t 
        (progn
          ;; first check the preconditions, PIF any
          (when preconditions
            ...
            (PUNLESS pu
              (trace-print :operators (first head)
                           ...
                           (first head)
                           task-body)
              (RET apply-operator (values 'fail preconditions 0))))
...
;(format t "~%Sat to explain: ~s" (apply-substitution preconditions unifier))
	  ...
	  (PIF *explanation*
          ...
          (setq cost-number (PIF (numberp cost-value) cost-value 1))
          ;; process DELETE list
          (CDOLIST (d dels-subbed)
            (PUNLESS (eql 'fail d)
              (PIF (eql (car d) 'forall)
                (CLET ((bounds (third d)) (dels (fourth d)) mgu2 tempd)
                  (setq mgu2 (find-satisfiers bounds state))
                  (CDOLIST (m2 mgu2)
          ...
          ;; process ADD list
          (CDOLIST (a adds-subbed)
            (PUNLESS (eql 'fail a)
              (PIF (eql (car a) 'forall)
                (CLET ((bounds (third a)) (adds (fourth a)) mgu2 tempa)
                  (setq mgu2 (find-satisfiers bounds state))
                  (CDOLIST (m2 mgu2)
...
(defun apply-method (state task-body method protections depth)
  ...
  (CLET ((standardized-method (standardize method))  
        task-unifier state-unifiers pre tail)

    ;; see PIF the standardized-method's head unifies with TASK-BODY
    (setq task-unifier (unify (second standardized-method) task-body))
    (PUNLESS (eql task-unifier 'fail)
      ;; STANDARDIZED-METHOD's CDDR is a list 
      ;; (label_1 pre_1 d_1 label_2 pre_2 d_2 ...) which acts like an
      ;; PIF-then-else: we look for the first true pre_i, and then evaluate d_i
...
(defun already-immediate-p (reduction)
  (PCOND
   ...
   ((eq (first reduction) :unordered)
    (find-PIF #'already-immediate-p (rest reduction)))))

(defun force-immediate-unordered (unordered-list &optional previous)
  (PIF (null unordered-list)
      nil
    (append
      ...
      (ecase (car x)
        ...
        ((:operator)
         (PIF (gethash (caadr x) operators)
           ...
           (push (process-operator x) (gethash (caadr x) operators))))
        ((:-) 
         (setf x (regularize-axiom x))          ; convert SHOP 1.x axioms into SHOP 2 axioms PIF necessary
         (push x (gethash (caadr x) axioms))))) 
    (setq *axioms* axioms)
    (setq *operators* operators)
    (setq *methods* methods)))

;;; If the axiom has SHOP 1.x COR mixed syntax, regularize-axiom will return it in SHOP 2 syntax.
(defun regularize-axiom (axiom)
  (PIF (shop2-axiom-p axiom) axiom       ; SHOP2 syntax
    (CLET ((ax-name (caadr axiom))       ; SHOP1 COR mixed syntax
          (branch-counter 0)
          ...
          (append labelified-axiom 
              (loop until (null atail) do
                    (incf branch-counter)
                    PIF (listp (car atail))       ; no label on this branch
...
(defun shop2-axiom-p (ax)
  (PIF (not (listp ax)) nil
      (CLET ((lax (length ax)))
        (PIF (<= lax 3) nil (rest-shop2-axiom-p (cddr ax))))))

(defun rest-shop2-axiom-p (ax)
  (CLET ((lax (length ax)))
    (PCOND ((< lax 2) nil)
          ((= lax 2) (PIF (and (atom (car ax)) (listp (cadr ax))) t nil))
          ((= lax 3) nil)
          (t (and (atom (car ax)) (listp (cadr ax)) (rest-shop2-axiom-p (cddr ax)))))))
  
(defun set-variable-property (x)
  (PCOND ((atom x) (and (symbolp x) 
                       (COR (and (equal (elt (symbol-name x) 0) #\?)
...
(defun make-problem (problem-name state tasks &optional extra)
  ;; PIF extra is given, then the args are problem-name, domain-name, state, tasks
...
(defun get-state (name)
  (CLET ((answer (get name :state 'fail)))
    (PIF (eq answer 'fail) (error "No initial state for the name ~s" name))
    answer))

;;; Get the task list for the problem named NAME
(defun get-tasks (name)
  (CLET ((answer (get name :tasks 'fail)))
    (PIF (eq answer 'fail) (error "No task list for the name ~s" name))
    answer))

;;; Get the list of problems for the problem set named NAME
(defun get-problems (name &key print)
  (CLET ((answer (get name :problems 'fail)))
    (PCOND ((eq answer 'fail) (error "No problem list for the name ~s" name))
          (print (format t "~%~s" answer)))
    answer))

;;; DO-PROBLEMS runs FIND-PLANS on each problem in PROBLEMS, which may be
;;; either a problem-set name COR a list of problems
(defun do-problems (problems &rest keywords)
  (PIF (not (listp problems))    ; treat NIL as an empty list, not a problem name
    (setq problems (get-problems problems)))
  (CDOLIST (problem problems)
...
;;; efficiency tweak PIF you have a domain with lots of unordered tasks which
;;; reduce to iNOP's).  - Murdock 12/01
(defun get-immediate-list (tl)
  (CDOLIST (obj tl)
    (PIF (COR (eq :immediate (second obj)) (eq '!!inop (second obj))) 
      (RET get-immediate-list (list obj))))
  (RET get-immediate-list nil))
...
(defun seek-plans (state tasks top-tasks partial-plan partial-plan-cost
  ...
  (when (time-expired-p)
    (PIF *print-stats* (format t "~%Terminating because the time limit has expired."))
  ...
  (PCOND
    ;; PIF no top-tasks to accomplish, then we have an answer
    ((COR (null top-tasks) (equal top-tasks '(NIL)))
      (seek-plans-null state which-plans partial-plan partial-plan-cost depth))
   
    ;; else PIF we've hit the depth bound, then fail
    ((and *depth-cutoff* (>= depth *depth-cutoff*)))

    ;; else look at the agenda
    (t
     (CLET* ((immediate-tasks (get-immediate-list top-tasks))
            task1)
       (PIF immediate-tasks
         (progn
...
(defun seek-plans-primitive (task1 task-name task-body state tasks top-tasks
  ...
  (multiple-value-bind
   ...
   (when (eql result1 'fail)
    ...
    (CLET ((new-cost (+ cost partial-plan-cost)))
...
(defun seek-plans-nonprimitive (task1 task-name task-body state tasks 
                                      ...
                                      depth which-plans protections)
  (CLET (result1 tasks1 top-tasks1 label r)
    (CDOLIST (m (gethash task-name *methods*))
	  ...
	  (when *plan-tree* (record-reduction task1 r))
          ...
          (and *plans-found* (eq which-plans :first)
               (not (optimize-continue-p))
               (RET seek-plans-nonprimitive nil)))))))
      ...
      (when (and *optimize-cost* acceptable-cost)
        ...
        (PIF (member which-plans '(:first :shallowest))
          ...
          (setq *plans-found*
                 ...
                 *plans-found*))))
      (PCOND
       ((COR (eq which-plans :all-shallowest)
...
(defun record-reduction (task1 reduction)
  (CLET ((all-subtasks (extract-subtasks reduction)))
...
(defun extract-tree (plan)
  (strip-tree-tags
   (CLET* ((operator-nodes (plan-operator-nodes plan))
...
(defun extract-subtree (root-node nodes)
  (CLET ((children (node-children root-node nodes)))
    (PIF children
	(cons root-node
...
(defun node-children (node nodes)
  (remove-PIF-not
   #'(lambda (other-node) 
       (eq (second (assoc (COR (operator-task other-node) other-node)
			  *subtask-parents*))
	   node))
   nodes))

(defun plan-tree-nodes (base-nodes)
  (CLET* ((extended-base-nodes
	  (remove-duplicates
...
(defun extend-plan-tree-nodes (base-nodes)
  (PIF (null base-nodes) nil
    (CLET* ((operator-task (operator-task (first base-nodes)))
	   (task (COR operator-task (first base-nodes)))
...
(defun print-stats (depth plans tasks inferences runtime realtime)
  (PIF *print-stats*
    (format t "~%~6@a~6@a ~7@a ~7@a~11@s~11@s~10,3f~11,3f"
            depth (length plans)
            (PIF plans (to-string (apply #'min (mapcar #'plan-cost plans)) 2) "-")
            (PIF plans (to-string (apply #'max (mapcar #'plan-cost plans)) 2) "-")
            tasks inferences
...
(defun to-string (num &optional (max-decimal 1))
  (PIF (integerp num) (format nil "~a" num)
    (CLET* ((base-string (format nil (format nil "~~,~aF" max-decimal) num))
	   (trimmed-string (string-right-trim "0" base-string)))
      (PIF (eq #\. (char trimmed-string (1- (length trimmed-string))))
...
(defun internal-operator-p (operator-name)
  (PIF (symbolp operator-name) 
      (CLET ((name (symbol-name operator-name)))
        (and 
         (>= (length name) 2)
         (equal (elt name 0) #\!)
         (equal (elt name 1) #\!)))
...
(defun print1 (obj)
  (PIF (atom obj) (format t " ~A" obj)
      (progn
        (format t "(~S" (car obj))
        (CDOLIST (obj1 (cdr obj))
          (print1 obj1))
        (format t ")"))))
...
;;; be invoked in call COR eval forms within a SHOP domain; they are
...
(defun process-pre (pre)
  ...
  (PCOND  
   ...
   ((eql pre1 'COR)
    (setq answer (cons 'COR (process-pre (cdr pre)))))
   ((eql pre1 'imply)
    ...
    (append answer 
            (loop until (null tail) do  
                  ...
                  append (list  (process-pre (pop tail))
                                ;; check to see PIF there is a quote COR 
                                ;; backquote in the front of this list (SHOP1
                                ;; COR SHOP2 syntax) and process accordingly
                                (progn 
                                  (setq clause (pop tail)
                                        first-of-clause (car clause))
                                  (PCOND ((COR (eq first-of-clause 'quote)
...
;;; returns t PIF item is found anywhere in the tree; doubly recursive,
;;; but only runs once per method definition.
(defun search-tree (item tree)
  (PCOND ((eq item tree) t)
        ((not (consp tree)) nil)
        (t (COR (search-tree item (car tree)) (search-tree item (cdr tree))))))
...
;;; backquote is done COR the quote is just taken off.  Now, something like backquote needs to be done.
;;; perhaps the form should be searched for an instance of "call"; PIF found, put "backquote" on the
...
(defun process-operator (operator)
  (CLET ((lopt (length operator)))
    (append (list (first operator) (second operator))
        (PCOND ((= lopt 4)             ; a SHOP 1 operator, no cost specified
               (list '() 
...
;;;      0 COR NIL    - print nothing
;;;      1 COR :STATS - print some statistics on SHOP2's operation
;;;      2 COR :PLANS - print the stats and print all plans found, but omit
;;;           operator costs and omit all operators whose names start with "!!"
;;;      3 COR :LONG-PLANS - print the stats and plans, including all operator
...
(defun find-plans (problem
  ...
  #+(COR :MCL :allegro)(PIF gc (gc))
  (CLET* ((*start-run-time* (get-internal-run-time))
         (*start-real-time* (get-internal-real-time))
         (*internal-time-limit* 
          (PIF time-limit (* time-limit
       ...
       (do ((*depth-cutoff* 0 (1+ *depth-cutoff*)))
         ...
         (catch-internal-time
          (seek-plans state tasks top-tasks nil 0 0 
                      (PIF (eq which :id-first) :first :all) nil))
         ...
         (and (equal *expansions* old-expansions)
              ...
              (return nil))  ; abort PIF nothing new happened on this iteration
    ...
    (CLET ((plan-trees
      ...
      (values (PIF *plan-tree* (list plan-trees *plans-found*) *plans-found*)
              (+ 0.0
                 (/ total-run-time
                    internal-time-units-per-second))))))
