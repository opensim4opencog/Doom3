...
* they appear the same (e.g., same hilight, same check state) for all menu bar copies.
*
 ...
 */ 
 public void 		updateToolMenuDebug(boolean dbg, boolean unkpred,
 						boolean trce,boolean brk, boolean in_query)
 {Enumeration 		e = kb_windows.elements();
  
  while (e.hasMoreElements())
  {
   updateToolMenuDebug(((gConsultWindow) e.nextElement()).getMenuBar(), dbg,unkpred,trce,brk,in_query);
  }
 };
 ...
 */ 
 protected void 	updateToolMenuDebug(MenuBar menubar,boolean dbg,
						boolean unkpred,boolean trce,boolean brk, boolean in_query)
 ...
 {Enumeration 		e = kb_windows.elements();
  ...
  while (e.hasMoreElements())
  {
   updateToolMenuQuery(((gConsultWindow) e.nextElement()).getMenuBar(), in_query,paused);
  }
 };

 protected void 	updateToolMenuQuery(MenuBar menubar,boolean in_query, boolean paused)
 {Menu			tool_menu = menubar.getMenu(getToolMenuID());
  int 			i, max = tool_menu.getItemCount();
 ...
 {Enumeration 		e = kb_windows.elements();
  ...
  while (e.hasMoreElements())
  {
   updateToolMenuConsultAll(((gConsultWindow) e.nextElement()).getMenuBar(), state);
  }
 };
