...
#else  // cusTom3 - needed #else to keep original functionality here ;(
		gameLocal.Error( "Unknown ammo type '%s'", ammoname );
#endif
	}
...
#ifdef _D3XP // cusTom3 - xp change not originally wrapped - looks functionally equivalent anyway
				&& ( (gameLocal.gameType != GAME_TDM ) || gameLocal.serverInfo.GetBool( "si_teamDamage" ) || ( owner->team != static_cast< idPlayer * >( ent )->team ) )
#else  // original version didn't have extra parens, he he
				&& ( gameLocal.gameType != GAME_TDM || gameLocal.serverInfo.GetBool( "si_teamDamage" ) || ( owner->team != static_cast< idPlayer * >( ent )->team ) )
#endif
				) {
