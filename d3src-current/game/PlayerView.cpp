...
#ifdef _D3XP // cusTom3 - xp changed all gameLocal.times (what is the use of #ifdefs when you do stuff like this half the time?)
...
void idPlayerView::DamageImpulse( idVec3 localKickDir, const idDict *damageDef ) {
	...
	//
#ifdef _D3XP // cusTom3 - xp changed all gameLocal.times
...
/*
	...
	screenBlob_t *blob = GetScreenBlob();
	// TODO: cusTom3 - if this is ever uncommented it will need to be wrapped for gameLocal.time
	blob->startFadeTime = gameLocal.slow.time;
...
idAngles idPlayerView::AngleOffset() const {
	...
	ang.Zero();
#ifdef _D3XP // cusTom3 - xp changed all gameLocal.times - it is much nastier to read this way, but i am trying to get it to compile :( without defines
...
#else	// cusTom3 - originally this was just called - when _D3XP is undefined it still should call it?
...
#ifdef _D3XP // cusTom3 - xp changed all gameLocal.time
...
#ifndef _D3XP // cusTom3 - this was removed from the d3xp version but in the game version
