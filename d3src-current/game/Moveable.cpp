...
void idMoveable::Spawn( void ) {
	...
	int clipShrink;
	idStr clipModelName="                                                                                   "; //dmiles added ="";
	...
	idStr tcm = clipModelName;
	bool bcck  = collisionModelManager->TrmFromModel( clipModelName.c_str(), trm );

	if ( !bcck ) {
// dmiles changed to only warning 
...
bool idMoveable::Collide( const trace_t &collision, const idVec3 &velocity ) {
	...
	// TODO: cusTom3 - this is a mess - monsterDamage (declared only when _D3XP) is everywhere in this :(
#ifdef _D3XP // cusTom3 - wrapping above said changes in #ifdef because monsterDamage is declared inside one and i want to build
	...
	if ( canDamage && damage.Length() && gameLocal.time > nextDamageTime ) {
			...
			if ( ent && v > minDamageVelocity ) {
				...
				dir = velocity;
				dir.NormalizeFast();
#ifdef _D3XP // cusTom3 - moved this up a line above the if right below - hasMonsterDamage is _D3XP specific
...
// #else - cusTom3 removed #else, code, and #endif
				// ent->Damage( this, GetPhysics()->GetClipModel()->GetOwner(), dir, monsterDamage, f, INVALID_JOINT );
// #endif
				} else if ( hasDamage ) {
// #ifdef _D3XP - cusTom3 removed - still in the one i moved above
...
#else
					ent->Damage( this, GetPhysics()->GetClipModel()->GetOwner(), dir, damage, f, INVALID_JOINT );
#endif // cusTom3 - moved down one line to after the closing brace for the if now included above

				nextDamageTime = gameLocal.time + 1000;
			} // ( ent && v > minDamageVelocity ) {
#ifdef _D3XP
		} // if ( hasDamage || hasMonsterDamage ) {
#endif
	} // if ( !gameLocal.isClient && canDamage && gameLocal.time > nextDamageTime ) { OOOORRRRR if ( canDamage && damage.Length() && gameLocal.time > nextDamageTime ) {
#ifdef _D3XP
