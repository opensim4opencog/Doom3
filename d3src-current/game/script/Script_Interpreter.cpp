...
//cycscript "sys.evalString($player1,%22hello%22)"
//-------------------------------------------------------------------------------------- 
// Description: Push the specified integer value onto the stack. 
// Parameters: The integer value to push onto the stack. 
// Returns: None. 
//-------------------------------------------------------------------------------------- 
void idInterpreter::PushInt(int nValue) {
	// Integers are treated as floats. 
	PushFloat(nValue); 
} 
//-------------------------------------------------------------------------------------- 
// Description: Push the specified float value onto the stack. 
// Parameters: The float value to push onto the stack. 
// Returns: None. 
//-------------------------------------------------------------------------------------- 
void idInterpreter::PushFloat(float fValue) {
	PushValue(fValue); 
} 
//-------------------------------------------------------------------------------------- 
// Description: Push the specified vector value onto the stack. 
// Parameters: The vector value to push onto the stack. 
// Returns: None. 
//-------------------------------------------------------------------------------------- 
void idInterpreter::PushVector(const idVec3& k_rv3Value) {
	PushValue(k_rv3Value); 
} 
//-------------------------------------------------------------------------------------- 
// Description: Push the specified entity onto the stack. 
// Parameters: The entity to push onto the stack. 
// Returns: None. 
//-------------------------------------------------------------------------------------- 
void idInterpreter::PushEntity(const idEntity* k_pEntity) {
	if (k_pEntity) {
		PushValue(k_pEntity->entityNumber + 1); 
	} else {
		PushValue((int)NULL); 
	}
} 
...
//-------------------------------------------------------------------------------------- 
// Description: stack the variable arguments. 
// Parameters: The script function to call, 
// The format string (same as script event format strings), 
// The variable argument list. 
// Returns: the ammount of space used on stack. 
//-------------------------------------------------------------------------------------- 
int idInterpreter::PushCmdArgs(const char* strFormat, const idCmdArgs &args, int argNum) {
	// Store the old stack pointer. 
	int nOldLocalStackUsed = localstackUsed; 
	// Push the arguments onto the interpreter stack. 
	int nNumArguments = strlen(strFormat);
	//gameLocal.Printf("nNumArguments=%d (%s)",nNumArguments,strFormat);
	if (nNumArguments > 0) {
		// Get the arguments. 
		while (*strFormat) {
			const char* vstring = args.Argv(argNum++);
			char cArgumentType = *strFormat;
			// gameLocal.Printf("Get the %d argument %c = '%s'",argNum,cArgumentType,vstring);
			switch (cArgumentType) {
			case D_EVENT_INTEGER:{
					int tempvar;
					if (sscanf(vstring,"%d",&tempvar)<1) gameLocal.Printf( "(throw format_int \"%s\")\n", vstring );
					PushInt(tempvar);
					break; 
				}
			case D_EVENT_FLOAT:{
//                    double tempvar;
					float tempvar;
					if (sscanf(vstring,"%f",&tempvar)<1) gameLocal.Printf( "(throw format_float \"%s\")\n", vstring );
					PushFloat(tempvar);
					break; 
				}
			case D_EVENT_VECTOR:{                           
					PushVector(*chars_to_vector(vstring));
					break; 
				}
			case D_EVENT_STRING:{
					PushString(vstring);
			...
			case D_EVENT_ENTITY_NULL:{
					idEntity *check = SearchEntity(vstring);
					PushEntity(check);
					if (!check)	gameLocal.Printf( "(throw entity_not_found \"%s\")\n", vstring );
					break; 
				}
			default:{ 
					gameLocal.Printf("Invalid argument '%d' format type '%c' for script function.",argNum,cArgumentType); 
					break; 
				} 
	...
	return localstackUsed - nOldLocalStackUsed;
}
//-------------------------------------------------------------------------------------- 
...
// Returns:      None. 
//-------------------------------------------------------------------------------------- 
idThread* idInterpreter::CallFunction(const function_t* k_pfnScript, idStr strFormat, va_list& rArgumentList) {
	// Store the old stack pointer. 
	int nOldLocalStackUsed = localstackUsed; 

	// Push the arguments onto the interpreter stack. 
	int nNumArguments = strFormat.Length(); 
	if (nNumArguments > 0) {
		// Get the arguments. 
		const char* k_pszType = strFormat.c_str(); 
		while (*k_pszType) {
			// Get the current argument. 
	#define HANDLE_ARGUMENT_TYPE(_TypeChar, _Type, _Function) case _TypeChar:   { _Type Arg = va_arg(rArgumentList, _Type); _Function(Arg); } break 
			char cArgumentType = *k_pszType; 
			switch (cArgumentType) {
			HANDLE_ARGUMENT_TYPE(D_EVENT_INTEGER, int, PushInt); 
			HANDLE_ARGUMENT_TYPE(D_EVENT_FLOAT, double, PushFloat);		  // MUST be double, not float for va_arg(). 
			HANDLE_ARGUMENT_TYPE(D_EVENT_VECTOR, idVec3, PushVector); 
			HANDLE_ARGUMENT_TYPE(D_EVENT_STRING, const char*, PushString); 
			HANDLE_ARGUMENT_TYPE(D_EVENT_ENTITY, idEntity*, PushEntity); 
			HANDLE_ARGUMENT_TYPE(D_EVENT_ENTITY_NULL, idEntity*, PushEntity); 
			default:{ 
					Error("Invalid argument format type '%c' for '%s' script function.", cArgumentType, k_pfnScript->Name()); 
				} 
				break; 
			} 

	#undef HANDLE_ARGUMENT_TYPE 
	...
	// Set the size of parameters to pop when calling the function. 
	int nStackSpaceUsedByArguments = localstackUsed - nOldLocalStackUsed;
	// Call the function. 
	idThread* pThread = new idThread(this, k_pfnScript, nStackSpaceUsedByArguments); 
	assert(pThread); 
...
/*
================
idInterpreter::CallEvent->CallClassEvent
================
*/
void idInterpreter::CallClassEvent(idClass *obj, const idEventDef *evdef, int argsize ) {
	int                 i;
	int                 j;
	varEval_t           var;
	int                 pos;
	...
	bool doesRespondTo = obj->RespondsTo( *evdef );

	if (obj->IsType(idEntity::Type)) {
		eventEntity = static_cast<idEntity*>(obj);
	}
	if (obj->IsType(idThread::Type)) {
		thread = static_cast<idThread*>(obj);
	}

	if (!doesRespondTo) {
		if (developer.GetBool()) {
			// give a warning in developer mode
			Warning( "Function '%s' not supported on class '%s'", evdef->GetName(), obj->GetClassname() );
		}
	...
	for (j = 0, i = 0, pos = type_object.Size(); ( pos < argsize ) || ( format[ i ] != 0 ); i++) {
		...
		default :
			...
			// dmiles
		pos += charToTypeDef(format[j++])->Size();
	}

	popParms = argsize;

	obj->ProcessEventArgPtr( evdef, data );
...
/*
idThread* idInterpreter::class2Thread(idClass* check) {
	if (check->IsType(idThread::Type)) return static_cast<idThread*>(check);
	idEntity* entity = class2Entity(check);
	idThread* athread = idThread::CurrentThread();
	idInterpreter* interp = this;
	if (check) {
		idThread* athread = new idThread();
		athread->SetThreadName(va("%s_class2Thread_%d",entity->GetName(),athread->GetThreadNum()));
		athread->interpreter.eventEntity = entity;
	}
	if (eventEntity && eventEntity!=check) return class2Thread(eventEntity);
	if (!interp->thread)	interp->SetThread(athread);
	return athread;
}

idEntity* idInterpreter::class2Entity(idClass* check) {
	if (check->IsType(idEntity::Type)) return static_cast<idEntity*>(check);
	idThread* aThread = class2Thread(check);
	if (!check) {
		if (eventEntity) return eventEntity;
	} else {
		if (aThread) return aThread->interpreter.eventEntity;
		if (aThread->waitingFor!=ENTITYNUM_NONE) return gameLocal.entities[aThread->waitingFor];
		if (aThread->waitingForThread) return class2Entity(aThread->waitingForThread);
		if (gameLocal.program.returnDef) {
			if (gameLocal.program.returnDef->Type()==ev_entity)	return GetEntity(* gameLocal.program.returnDef->value.entityNumberPtr);
		}
	}
	if (eventEntity) return eventEntity;
	if (thread && thread!=check) return  class2Entity(thread);
	return eventEntity;
}

idVarDef* idInterpreter::InvokeEntity(idEntity* check,const function_t* func,const idCmdArgs &args) {
	return InvokeScript(check,class2Thread(check),func,args);
}
idVarDef* idInterpreter::InvokeThread(idThread* check,const function_t* func,const idCmdArgs &args) {
	return InvokeScript(class2Entity(check),check,func,args);
}
*/
idVarDef* idInterpreter::InvokeEvent(idClass* check,const idEventDef* evdef,const idCmdArgs &args) {
	idThread* savedThread = thread;
	idEntity* savedEventEntity = eventEntity;
	if (check==NULL) check = eventEntity;
	const char rtype = evdef->GetReturnType();
	idTypeDef* ftype = charToTypeDef(rtype);
	idVarDef* var = new idVarDef(ftype);
	idVarDef* savedReturnDef = gameLocal.program.returnDef;
	int nStackSpaceUsedByArguments = PushCmdArgs(evdef->GetArgFormat(),args,0);
	gameLocal.program.returnDef = var;
	CallClassEvent(check,evdef,nStackSpaceUsedByArguments);
	gameLocal.program.returnDef = savedReturnDef;
	thread = savedThread;
	eventEntity = savedEventEntity;
	return var;
}
...
//	gameLocal.Printf("InvokeScript: %s ",scriptSignature(func).c_str());

idVarDef* idInterpreter::InvokeScript(const function_t* func,const idCmdArgs &args) {
	if (func->eventdef)	return InvokeEvent(NULL,func->eventdef,args);
	idInterpreter* interpreter = this;
	idThread* savedThread = thread;
	idEntity* savedEventEntity = eventEntity;
	const idTypeDef *ftype = func->type;
	if (!ftype) {
		return toErrorDef(va("'%s' function missing Decls\n",func->Name()));
	}
	idVarDef* var = new idVarDef(ftype->ReturnType());
	idVarDef* savedReturnDef = gameLocal.program.returnDef;
	int nStackSpaceUsedByArguments = interpreter->PushCmdArgs(scriptFormat(func),args,0);
	gameLocal.program.returnDef = var;
	idThread* dThread = new idThread(interpreter,func,nStackSpaceUsedByArguments);
	if (var->Type()==ev_void) {
		dThread->DelayedStart(0);
	} else {
		dThread->ManualDelete();
		while (!dThread->IsDying() && dThread->IsDoneProcessing()) {
			dThread->Execute();
		}
		if (dThread && dThread!=thread)	delete dThread;
	}
	gameLocal.program.returnDef = savedReturnDef;
	thread = savedThread;
	eventEntity = savedEventEntity;
...
void idInterpreter::CallEvent( const function_t *func, int argsize ) {
	...
	for (j = 0, i = 0, pos = type_object.Size(); ( pos < argsize ) || ( format[ i ] != 0 ); i++) {
		...
		// dmiles confirm i havent changed this
			pos += func->parmSize[ j++ ];
...
====================
*/
void idInterpreter::PopParms( int numParms ) {
...
====================
*/
void idInterpreter::Push( int value ) {
...
====================
*/
void idInterpreter::PushString( const char *string ) {
...
====================
*/
const char *idInterpreter::FloatToString( float value ) {
	static char text[ 32 ];
...
====================
*/
void idInterpreter::AppendString( idVarDef *def, const char *from ) {
...
====================
*/
void idInterpreter::SetString( idVarDef *def, const char *from ) {
	if (def->initialized == idVarDef::stackVariable) {
...
====================
*/
const char *idInterpreter::GetString( idVarDef *def ) {
	if (def->initialized == idVarDef::stackVariable) {
...
====================
*/
varEval_t idInterpreter::GetVariable( idVarDef *def ) {
	if (def->initialized == idVarDef::stackVariable) {
		varEval_t val;
...
====================
*/
void idInterpreter::NextInstruction( int position ) {
