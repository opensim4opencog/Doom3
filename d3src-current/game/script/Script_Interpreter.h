...
#define MAX_STACK_DEPTH 	640
#define LOCALSTACK_SIZE 	61440
...
public: 
	...
	void CallClassEvent(idClass *obj, const idEventDef* evdef, int argsize );
	idVarDef* InvokeScript(const function_t* func,const idCmdArgs &args);
	idVarDef* InvokeEvent(idClass* check,const idEventDef* evdef,const idCmdArgs &args);
	idThread* CallFunction( const function_t* k_pfnScript, idStr strFormat, va_list& rArgumentList ); 
    ...
    int PushCmdArgs(const char* strFormat, const idCmdArgs &args, int argNum);
    template <typename t_Type> void PushValue(const t_Type& k_rValue); 
#endif// DAXMOO
