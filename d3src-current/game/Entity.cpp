...
#endif
#ifdef DAXMOO
const idEventDef EV_IsBoundTo( "isBoundTo", "E", 'd' );
const idEventDef EV_GetBindMaster( "getBindMaster", NULL, 'e' );
const idEventDef EV_GetBindJoint( "getBindJoint", NULL, 'd' );
#endif
...
#endif
#ifdef DAXMOO
EVENT( EV_IsBoundTo,        idEntity::Event_IsBoundTo )
EVENT( EV_GetBindMaster,        idEntity::Event_GetBindMaster )
EVENT( EV_GetBindJoint,     idEntity::Event_GetBindJoint )
#endif
END_CLASS


#ifdef DAXMOO
void idEntity::Event_IsBoundTo(idEntity* ent) {
	idThread::ReturnInt(IsBoundTo(ent));    
}
void idEntity::Event_GetBindMaster() {
	idThread::ReturnEntity(GetBindMaster());
}
void idEntity::Event_GetBindJoint() {
	idThread::ReturnInt(GetBindJoint());
}
#endif
...
#ifndef NOT_EVERDEFED
/*
================
idDeclModelDef::Parse
================

This preserves the inheritance infomation of the parent
*/
   ...
   // src.SetFlags( DECL_LEXER_FLAGS );
	...
	while (1) {
		...
		if (!token.Icmp("inherit")) {
			MessageBox(NULL, token2.c_str(), "Entity Inheritance", MB_OK);
			dict.Set("def_inherited",token2.c_str());
			idStr tokenRename = "def_inheriting ";
			tokenRename.Append(token2);
			dict.Set(tokenRename.c_str(),token2.c_str());
			const idDeclEntityDef *copy = static_cast<const idDeclEntityDef *>( declManager->FindType( DECL_ENTITYDEF, token2, false ) );
			if (!copy) {
				...
				//idDeclModelDef
				// parser.Warning( "'%s' inheriting '%s'", token.c_str(),token2.c_str() );
				dict.SetDefaults(&copy->dict );
			}
		}
...
#ifdef D3V_D3XP
const idEventDef EV_GetJointName( "getJointName", "d", 's' );
CLASS_DECLARATION( idEntity, idAnimatedEntity )
EVENT( EV_GetJointName,     idAnimatedEntity::Event_GetJointName )
#else
CLASS_DECLARATION( idEntity, idAnimatedEntity )
#endif
...
/*
================
idAnimatedEntity::Event_GetJointName

looks up the name of the specified joint.  returns INVALID_JOINT if the joint is not found.
================
*/
void idAnimatedEntity::Event_GetJointName( jointHandle_t joint ) {
	const char *jointname = animator.GetJointName( joint );
	idThread::ReturnString( jointname );
}
#endif// D3V_D3XP
