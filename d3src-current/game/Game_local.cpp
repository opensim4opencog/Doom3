...
#ifdef MOD_BOTS
botNetworkSystem *			networkSystem = NULL; // TinMan: Cobalts network wrapper
#else
...
#pragma export off
	...
	if ( import->version == GAME_API_VERSION ) {
		...
		#ifdef MOD_BOTS
		networkSystem				= new botNetworkSystem(import->networkSystem);  // TinMan: Cobalts network wrapper
		#else
...
void TestGameAPI( void ) {
	...
	#ifndef MOD_BOTS
	testImport.networkSystem			= ::networkSystem; // TinMan: *todo* Nerfed cause of network wrap
	#endif
...
#ifdef DAXMOO_EVENTS
	bool nowayjose = Daxmoo_Shutdown(); 
	if (nowayjose) return;
#endif
	if ( !common ) {
		return;
...
void idGameLocal::ShowTargets( void ) {
	...
	for( ent = spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next() ) {
#ifdef DAXMOO //The physics object is not yet valid
		if (!isValid(ent->GetPhysics())) {
			continue;
		}
#endif
...
#endif // cusTom3 - moved above NeedRestart()
...
#ifdef _D3XP // cusTom3 - no fast or slow with _D3XP
	int i = 0;
...
#ifdef _D3XP // cusTom3 - wrapped change and elsed to original
