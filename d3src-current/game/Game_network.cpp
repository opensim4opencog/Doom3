...
void idGameLocal::ServerClientDisconnect( int clientNum ) {
	...
	#ifdef MOD_BOTS
	// TinMan: remove bot entity *todo* test
	if ( botAi::bots[ clientNum ].inUse ) {
		idEntity * ent = entities[ botAi::bots[ clientNum ].entityNum ];
		if ( ent ) {
			delete entities[ botAi::bots[ clientNum ].entityNum ];
			botAi::bots[ clientNum ].inUse = false;
			botAi::bots[ clientNum ].clientID = -1;
			botAi::bots[ clientNum ].entityNum = -1;
		}
	}
	#endif
...
gameReturn_t idGameLocal::ClientPrediction( int clientNum, const usercmd_t *clientCmds ) {
	...
	memcpy( usercmds, clientCmds, BOT_START_INDEX * sizeof( usercmds[ 0 ] ) );
	#else
